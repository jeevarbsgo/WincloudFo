...
public class testclass {
	...
	//	takeScreenshot("Test_Sucessfull_Login");
		...
		@Test(dependsOnMethods = "loadLoginPage", priority = 1)
	public void TestRegpage() {
	...
	@Test(dependsOnMethods = "loadLoginPage", priority = 2)

	public void TestFoarrivaldate() {
	...
	/*
	@Test(dependsOnMethods = "loadLoginPage", priority=3)
	public void Testfowithtodaydate() {
		driver.get("https://test1dns.wincloudpms.net/TravelAgentBlock/FOReservation?VN=3.04.025");
	...
	@Test(dependsOnMethods = "loadLoginPage", priority = 4)
	public void TestFoRegistration() {
		driver.get("https://test1dns.wincloudpms.net/TravelAgentBlock/FOReservation?VN=3.04.025");
	...
	@Test(dependsOnMethods = "loadLoginPage", priority = 5)
	public void TestFoCheckin() throws InterruptedException, AWTException {
		driver.get("https://test1dns.wincloudpms.net/TravelAgentBlock/FOReservation?VN=3.04.025");
		WebElement add = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//div[@class='webix_el_box'])[2]")));
		add.click();
		WebElement arrivalDate = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@aria-label='Arrival']")));
	...
	@Test(dependsOnMethods = "loadLoginPage", priority = 1)
	public void Closethechatbox() throws InterruptedException {
		driver.get("https://test1dns.wincloudpms.net/TravelAgentBlock/FOReservation?VN=3.04.025");
		...
		// After hover, wait until the close button is visible
	//	WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
		//WebElement closeBtn = wait.until(ExpectedConditions.elementToBeClickable(By.className("zs-tip-close")));
	...
	public void Verify_Positive_nights() throws InterruptedException, AWTException {
	  ...
	  String Nights = nightsInput.getAttribute("value");
	   ...
	   Assert.assertEquals(Nights, "1");
	    ...
	    // Navigate to reservation search
	  /*  driver.navigate().to("https://test1dns.wincloudpms.net/TravelAgentBlock/FOReservation?MODE=C&VN=3.04.025");
	    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[@class=\" fa fa-folder-open\"]"))).click();
	    ...
	    Robot robot = new Robot();
	    robot.setAutoDelay(200);
	    for (char ch : reserveNoValue11.toCharArray()) {
	        typeCharWithRobot(robot, ch);
	    }

	    WebElement dynamicElement = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[text()='" + reserveNoValue11 + "']")));
	    new Actions(driver).doubleClick(dynamicElement).perform();
	    ...
	    WebElement regnum = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@style=\"text-align:center !important;font-weight:bold;width:100%;color:#fb2510\"]")));
	    String regText = regnum.getText();
	    System.out.println(regText);

	    Assert.assertTrue(regnum.isDisplayed(), "The alert element is not displayed!");
	    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("(//input[@type='checkbox'])[6]"))).click();
	    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[@class='webix_button webix_img_btn']"))).click();

	    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[text()=\"Rate Edit\"]"))).click();
	}
...
//		js1.executeScript("arguments[0].innerText = '';", DepartureDate);
	...
	/*private void takeScreenshot(String methodName) {
	    ...
	    // Custom/manual file path
	    String customFolderPath = "C:\\Users\\Jeeva\\Pictures\\ScreenShot";  // 👈 Set your desired path here

	    // Add timestamp to filename
	    String timestamp = java.time.LocalDateTime.now()
	            .format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
	    String filename = methodName + "_" + timestamp + ".png";
	    String fullPath = customFolderPath + File.separator + filename;
